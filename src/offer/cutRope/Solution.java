package offer.cutRope;

public class Solution {
    public int cutRope(int target) {
        if(target < 2)//因为要求长度n>1,所以这里返回0表示输入非法
            return 0;
        if(target == 2)//长度为2时,因为要求剪下段数m>1,所以最大是1x1=1
            return 1;
        if(target == 3)//长度为3时,因为要求剪下段数m>1,所以最大是1x2=2
            return 2;
        ...
        max = products[target];//将其记录下来以在销毁后能返回
        return max;
    }
}
